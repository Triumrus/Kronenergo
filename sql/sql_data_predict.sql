...
group by ID_COMPANY,ID_GTP,CITY_ID
union all
select DATE(DATE_ADD(max(DATE_ADD(CONVERT(DATE,DATETIME),INTERVAL HOUR HOUR)), interval 35 HOUR)) AS DATE, HOUR(DATE_ADD(max(DATE_ADD(CONVERT(DATE,DATETIME),INTERVAL HOUR HOUR)), interval 35 HOUR)) AS HOUR,ID_COMPANY,ID_GTP,CITY_ID from energo.fact_energoeffect
group by ID_COMPANY,ID_GTP,CITY_ID
)pree
)a
