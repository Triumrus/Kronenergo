...
data<- copy(data[(nrow(data)-35):nrow(data),])
...
data$pred_clf5<- predict(clf5,data.matrix(data[,-"FACT"]),ntreelimit = clf5$best_iteration)

names(data)[1:length(names_data)] <- names_data
data$HOUR<- 
  as.numeric(str_extract(
  apply(data[,(grep("^HOUR_[0-9]{1,2}$",names(data))),with=F],1,function(x){names(data[,(grep("^HOUR_[0-9]{1,2}$",names(data))),with=F])[which(x==1)]})
  ,"[0-9]{1,2}$"))
...
data$ID_COMPANY <- unique(ID_COMPANY)
data$ID_GTP <- unique(ID_GTP)
data$DATE<- date_pred[(length(date_pred)-35):length(date_pred)]
data$DATE <- as.character(data$DATE)


datainbase<- data[,.(PREDICT=pred_clf5,DATE=DATE,HOUR=HOUR,ID_COMPANY=ID_COMPANY,ID_GTP=ID_GTP)]
...
fwrite(datainbase,"predict.csv",sep = ';',dec = ',')
ftpUpload("predict.csv", "ftp://api:vfnhbo8934ykfjhg@185.220.32.98/clients/energoeffect/predict/predict.csv")
